...
use aimongus_types::server_to_client::{self, PublicRoom};
                    ...
                    return Some(server_to_client::VoteResult {
                        ...
                        points: {
                            ...
                            server_to_client::Points {
                                ...
                                tricking_players: if did_someone_vote_for_you {
                                    POINTS_FOR_TRICKING_PLAYER
                                } else {
                                    0
                                },
                            }
    ...
    fn get_inner_game_state_view(&self, identity: ClientIdentity) -> ClientGameState {
        ...
        match (&self.stage, turn) {
            ...
            (GameStage::Voting, Some(turn)) => ClientGameState::Voting {
                ...
                you_voted: turn.votes.contains_key(&identity),
            },
...
#[get("/public-rooms")]
fn public_rooms(sessions: &State<SessionsMap>) -> Json<Vec<PublicRoom>> {
    Json(
        sessions
            .0
            .read()
            ...
            .map(|s| PublicRoom {
                room_code: s.room_code.to_string(),
                number_of_players: s.players.len() as u8,
            })
            .collect(),
    )
}
...
#[get("/create-room?<identity>&<username>&<emoji>&<is_public>")]
fn create_room(
    identity: ClientIdentity,
    username: String,
    emoji: String,
    ...
    let broadcast = {
        ...
        for player in locked_session.players.iter_mut() {
            ...
            if did_someone_vote_for_you {
                player.score = player.score + POINTS_FOR_TRICKING_PLAYER
            }
            if guessed_the_bot {
...
fn rocket() -> _ {
    ...
    rocket::build()
        .manage(sessions)
        .mount("/", routes![create_room, join_room, public_rooms])
}
